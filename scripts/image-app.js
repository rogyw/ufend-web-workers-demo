...
(function(){
  ...
  function manipulateImage(type) {
    ...
    console.log('manipulateImage(): started.');
    var imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
    toggleButtonsAbledness();

    if (window.Worker) { // Check if Browser supports the Worker api
      //Prepare for repsonse from worker
      ...
      console.log('manipulateImage(): Passing data to worker.');
      worker.postMessage([type, imageData]); // send data to our worker
      return;
    }
    else {
      //browser does not supporting Workers so fall back to handle
